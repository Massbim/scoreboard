...
const Prematch2 = () => {
  ...
  const handleNoChange = (e, index) => {
    ...
    setPlayerAway((prevPlayerAway) => {
      const updatedPlayerAway = [...prevPlayerAway];
      updatedPlayerAway[index] = { ...updatedPlayerAway[index], no: value };
      return updatedPlayerAway;
    });
  };
  const newPlayer = {
    name: "",
        ...
        playerAway.map(async (player) => {
          ...
          if (player.photo) {
            const clearPhotoPromise = axios.put(
              `${process.env.NEXT_PUBLIC_DATABASE_URL}/playerAway/${player._id}/photoDelete`
            );
  ...
  const renderForms442Away = () => {
    ...
    const handleClearFormClick = (index) => {
      setPlayerAway((prevPlayerAway) => {
        // Create a new array with the same length as playerAway
        const newPlayerAway = prevPlayerAway.map((player, i) => {
          if (i === index) {
        ...
        return newPlayerAway;
      });

      // Also, directly delete the photo from the server
      const playerId = playerAway[index]._id;
      axios
        .put(
          `${process.env.NEXT_PUBLIC_DATABASE_URL}/playerAway/${playerId}/photoDelete`
        )
        .then(() => {
